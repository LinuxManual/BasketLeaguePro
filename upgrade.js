...
function mountUltraPanel() {
  ...
  host.innerHTML = `
    <article class="panel ultra-card">
      <div class="panel-heading"><div><p class="section-label">Ultra Mode</p><h2>Command Center</h2></div><span class="pill">v7.2</span></div>
      <div class="command-row">
        <input id="ultra-command" placeholder="Γράψε εντολή: /focus /export /theme /backup /zen" maxlength="120" />
        <button id="ultra-run" type="button">Run</button>
      </div>
      <p class="muted-line">Shortcuts: <kbd>Ctrl</kbd>+<kbd>K</kbd> palette, <kbd>Ctrl</kbd>+<kbd>J</kbd> matches, <kbd>Ctrl</kbd>+<kbd>/</kbd> focus search.</p>
...
function stateFromStore() {
  ...
  try { return JSON.parse(raw); } catch { return { rosters: {}, matches: [], messages: [] }; }
}
...
function updateUltraStats() {
  ...
  const players = ((state.rosters?.HotHeroes?.length || 0) + (state.rosters?.['Ιπτάμενοι']?.length || 0));
  const avg = completed.length ? Math.round(completed.reduce((a,m)=>a+m.hotScore+m.flyScore,0)/completed.length) : 0;
  const closeGames = completed.filter(m => Math.abs(m.hotScore - m.flyScore) <= 5).length;
  const stats = [['Players', players], ['Matches', (state.matches||[]).length], ['Finals', completed.length], ['Avg pace', avg], ['Clutch', closeGames]];
  ...
  const w = Math.max(320, Math.floor(bounds.width * ratio));
  const h = Math.max(160, Math.floor(bounds.height * ratio));
  ...
  ctx.beginPath();
  data.forEach((v, i) => {
    const x = 20 + (i * (w - 40)) / Math.max(1, data.length - 1);
...
function runCommand(cmd) {
  ...
  if (v.includes('/zen')) { document.body.classList.toggle('zen'); logLine('Zen mode toggled'); return; }
...
function mountPalette() {
  ...
  modal.innerHTML = '<div class="palette-inner"><input id="palette-input" placeholder="Type command..." /><div class="palette-hint">/theme · /export · /focus · /backup · /zen</div></div>';
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.ctrlKey && e.key === '/') { e.preventDefault(); $('#match-search')?.focus(); }
...
function initUltraUpgrade() {
  ...
  $('#ultra-command')?.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { e.preventDefault(); $('#ultra-run')?.click(); }
  });
